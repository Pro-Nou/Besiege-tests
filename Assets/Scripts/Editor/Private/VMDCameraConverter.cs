...
public class VMDCameraConverter
{
	...
	//都是以左下角为原点
	Vector2[] GetInterpolationPoints(byte[] interpolation, int type)
	{
		int row = (type - 1) * 4;
	...
	Vector2[]  ConvertToFramekeyControllerPoint(Vector2 p1, Vector2 p2, Keyframe outKeyframe, Keyframe inKeyframe)
	{
	...
	private static float Tangent(Vector2 from, Vector2 to)
	{
		Vector2 vec = to - from;
		return vec.y / vec.x;
	}

	private static float Weight(Vector2 from, Vector2 to, float length)
	{
...
//		inKeyframe.weightedMode = WeightedMode.Both;
		...
		else
		{
			...
			var ptTuple = ConvertToFramekeyControllerPoint(intTuple[0], intTuple[1], outKeyframe, inKeyframe);    //转化为keyFrame的控制点
			p1 = ptTuple[0];
			p2 = ptTuple [1];
