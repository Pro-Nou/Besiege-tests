Shader "Unlit/3to2_Opaque"
{
	Properties
	{
		...
		_DisplacementColor ("DisplacementColor", Color) = (0.0,0.0,0.0,1.0)
	...
	SubShader
	{
		...
		{	
			...
			{
				v2f o;
				float _length = length(_WorldSpaceCameraPos - mul(unity_ObjectToWorld, v.vertex).xyz);
				float4 vertexCache = v.vertex;
				vertexCache.xyz += normalize(v.normal) * _Displacement * min(5,  _length);
				o.pos = mul(UNITY_MATRIX_MVP, vertexCache);
			...
			{
				fixed4 final = _DisplacementColor;
				return final;
			}
			ENDCG
		}
		Pass
		{
			...
			v2f vert (appdata v)
			{
				v2f o;
				//float4 vertexCache = v.vertex;
				//vertexCache.xyz += normalize(v.normal) * _Displacement;
